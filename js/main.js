...
function handleKey(k){
  ...
  if (G.state==='playing'){
    ...
    if (k==='v'){ striderVar=(striderVar+1)%ANIM_VARS.length;
      if(planet.striders&&planet.striders[0]) planet.striders[0].var=striderVar;
...
function buildStrider(){
  if(planet.striders || planet.strider) return;
  if(glbCache['strider']){
    const place=(px,pz,ry,vi)=>{
      const gl=tryPlaceGLB('strider',150); if(!gl) return null;
      const inner=gl.children[0]; if(inner) inner.rotation.y=-Math.PI/2;
      gl.traverse(m=>{ if(m.isMesh&&m.material&&m.material.color) m.material.color.multiplyScalar(1.1); });
      gl.position.set(px, duneH(px,pz), pz); gl.rotation.y=ry;
      gl.userData.t=0; gl.userData.wings=[]; gl.userData.var=vi; scene.add(gl);
      return {gl, mx:playClip(gl,'walk'), var:vi}; };
    const first=place(-1000,-900,.8,1); // B - heavy grazer
    if(first){ planet.strider=first.gl; planet.striders=[first];
      const second=place(-1340,-680,2.6,4); // E - full dress, second grazer further out (user: play with both variants)
      if(second) planet.striders.push(second);
      planet.striderMixer=first.mx; return; }
  }
  if(!buildStrider.giveUp){
    if(!buildStrider.timer){ buildStrider.timer=true;
      setTimeout(()=>{ buildStrider.giveUp=true; buildStrider(); },6000); }
    setTimeout(buildStrider,450); return; }
  const g=new THREE.Group();
...
function updatePlanetFx(dt){
  ...
  if(planet.striders){ for(const s of planet.striders){ s.mx.timeScale=ANIM_VARS[s.var].gait; s.mx.update(dt); } }
  else if(planet.striderMixer){ planet.striderMixer.timeScale=ANIM_VARS[striderVar].gait; planet.striderMixer.update(dt); }
  ...
  if(planet.striders){ for(const s of planet.striders){ s.gl.userData.t+=dt; driveStrider(s.gl, s.var, s.gl.userData.t); } }
  if(planet.strider){
    const st=planet.strider; if(!planet.striders) st.userData.t+=dt;
    if(!planet.striders) driveStrider(st, striderVar, st.userData.t);
...
function setPlanetary(on){
  ...
  if (on){
    ...
    if(planet.striders) planet.striders.forEach(s=>s.gl.visible=true); else if(planet.strider) planet.strider.visible=true;
    ...
    if (planet.striders) planet.striders.forEach(s=>s.gl.visible=false); else if (planet.strider) planet.strider.visible=false;
